...
# set linetype = 'p' for one factor plots to remove lines between factor levels
# if desired
                          ...
                          ...)
{
  ...
  if (n.predictors == 1) {
    ...
    if (is.factor(x[, 1])) {
      ...
      tickmarks <- if (type == "response")
                     make.ticks(ylim, link = trans.link, inverse =
                                trans.inverse, at = ticks$at, n = ticks$n)
                   else make.ticks(ylim, link = I, inverse = I, at = ticks$at, n
                                   = ticks$n)

      levs <- levels(x[, 1])

      plot <- xyplot(
                     eval(parse(text = paste("fit ~ as.numeric(", names(x)[1], ")"))),
      ...
      ylim <- if (!missing(ylim)) ylim else c(range[1] - 0.025 * (range[2] -
                                                                  range[1]),
                                              range[2] + 0.025 * (range[2] -
                                                                  range[1]))

      if (type == "response") {
        tickmarks <- make.ticks(ylim, link = trans.link, inverse =
                                trans.inverse, at = ticks$at, n = ticks$n)
      } else {
        tickmarks <- make.ticks(ylim, link = I, inverse = I, at = ticks$at, n =
                                ticks$n)
      }
      ...
      xlm <- if (nm %in% names(xlim)) { xlim[[nm]] } else range.adj(x[nm])
      ...
      plot <- xyplot(
                     eval(parse(text = paste("fit ~ trans(", names(x)[1], ")"))),
                     ...
                     panel = function(x, y, x.vals, rug, lower, upper, has.se, ...) {
                       ...
                       if (has.se) {
                         if (ci.style == "bars") {
                           larrows(x0 = x[good], y0 = lower[good], x1 = x[good],
                                   y1 = upper[good], angle = 90, code = 3, col =
                                   eval(colors[2]), length = 0.125 * cex/1.5)
                         } else {
                           if (ci.style == "lines") {
                             llines(x[good], lower[good], lty = 2, col =
                                    colors[2])
                             llines(x[good], upper[good], lty = 2, col =
                                    colors[2])
                           }
                         }
                       }
                       ...
                       if (has.thresholds) {
                         ...
                         panel.text(rep(current.panel.limits()$xlim[1],
                                    ...
                                    threshold.labels, adj = c(0, 0), cex = 0.75)
                         panel.text(rep(current.panel.limits()$xlim[2],
                                        length(thresholds)), thresholds,
                                    threshold.labels, adj = c(1, 0), cex = 0.75)
                       }
                     ...
                     has.se = has.se,
                     data = x,
                     scales = list(y = list(at = tickmarks$at, labels =
                                            tickmarks$labels, rot = roty),
                                   x = list(at = tickmarks.x$at, labels =
                                            tickmarks.x$labels, rot = rotx),
  ...
  if (multiline) {
    ...
    if (is.factor(x[, x.var])) {
      ...
      plot <- xyplot(
                     eval(parse(text = paste("fit ~ as.numeric(",
                     ...
                     panel = function(x, y, subscripts, z, lower, upper, show.se, ...) {
                       ...
                       for (i in 1:length(zvals)) {
                         ...
                         if (show.se) {
                           larrows(x0 = x[sub][good] + os,
                                   y0 = lower[subscripts][sub][good],
                                   x1 = x[sub][good] + os,
                                   y1 = upper[subscripts][sub][good],
                                   angle = 90,
                                   code = 3,
                                   col = 'black',
      ...
      tickmarks.x <- if ((nm %in% names(transform.x)) &&
        ...
        trans <- transform.x[[nm]]$trans
        make.ticks(trans(xlm), link = transform.x[[nm]]$trans, inverse =
                   transform.x[[nm]]$inverse, at = ticks.x$at, n = ticks.x$n)
      } else {
        trans <- I
        make.ticks(xlm, link = I, inverse = I, at = at, n = n)
      }
      ...
      plot <- xyplot(
                     eval(parse(text = paste("fit ~trans(",
                                             predictors[x.var], ")", if (n.predictors > 2)
                                               paste(" |", paste(predictors[-c(x.var, z.var)])),
                                             collapse = "*"))),
                     strip = function(...) strip.default(..., strip.names = c(factor.names, TRUE)),
                     panel = function(x, y, subscripts, x.vals, rug, z, lower, upper, show.se, ...) {
                       ...
                       for (i in 1:length(zvals)) {
                         ...
                         llines(x[sub][good], y[sub][good], lwd = 2, type = "l",
                                col = colors[i], lty = lines[i], cex = cex, ...)

                         if (show.se) {

                            os <- (i - (length(zvals) + 1)/2) * (2/(length(zvals) - 1)) * 0.01 * axis.length
                     ...
                     scales = list(y = list(at = tickmarks$at, labels = tickmarks$labels),
                                   rot = roty,
                                   x = list(at = tickmarks.x$at,
                                            labels = tickmarks.x$labels,
                                            rot = rotx),
                                   alternating = alternating),
                     ...
                    )
  ...
  tickmarks <- if (type == "response")
                 make.ticks(ylim, link = trans.link, inverse = trans.inverse, at
                            = ticks$at, n = ticks$n)
               else
                 make.ticks(ylim, link = I, inverse = I, at = ticks$at, n =
                            ticks$n)
    ...
    plot <- xyplot(eval(parse(text = paste("fit ~ as.numeric(",
                   ...
                   panel = function(x, y, subscripts, lower, upper, has.se, ...) {
                     ...
                     if (has.se) {
                       if (ci.style == "bars") {
                          larrows(x0 = x[good],
                                  y0 = lower[subscripts][good],
                                  x1 = x[good],
                                  y1 = upper[subscripts][good],
                                  angle = 90,
                                  code = 3,
                                  col = colors[2],
                                  length = 0.125 * cex/1.5)
                          ...
                          if (ci.style == "lines") {
                            llines(x[good],
                                   lower[subscripts][good],
                                   lty = 2,
                                   col = colors[2])
                            llines(x[good],
                                   upper[subscripts][good],
                                   lty = 2,
                                   col = colors[2])
                          }
                        }
                     ...
                     llines(x[good], y[good], lwd = 2, type = "b",
