...
# we need this function because I am taking input$analysis nd directly sending that to the call() function. This way I don't have to recode a bunch of stuff, I can have this function do it for me. It also prevents a bunch of different custom analyses for each analysis type. I don't have to have something like switch(input$analysis, ...) and do a separate analysis for each input.  Instead, it is just, call(input$analysis, args_to_function). Any unnecissary arguments are pulled out of the function call with this function. 
strip.args <- function(x){
  ...
  # if a distribution family is sellected AND if that family is gaussian, make the call "lm" rather than "glm"
  if(length(x$family) > 0 && x$family=='gaussian'){
    x[[1]] <- as.name('lm')
  }
  
  #convert the function being called into a string (i.e. "glm", "lm", "aov", or "t.test")
  the.call <- deparse(x[[1]])
  
  # if the call is "rcbd" as pulled from the input$select_analysis, make it an ANOVA because RCBD is an anova, people just seemto want a separate table for that analysis. 
  if(the.call=='rcbd'){
    x[[1]] <- as.name('aov')
  }

  # get rid of whatever calls are unnecissary ("family" in all but glm) for the analysis so evaluating the calls isn't weird.
  # all analysis functions for now have "..." so this won't affect the analysis, but we don't want "family" showing up in t.test because that is ridiculous.
...
# function for formatting p-values - used in xtable.htest so that p-values displayed in the table are beautiful. 
...
# generic function for xtable with t-tests in the report since xtable doesn't have a generic function for class "htest" (i.e. a t-test object)
...
# this is different from the typical paste function because we need to strip out NA values from the call to paste. Needed so when interactions are NA, we dont have a formula that looks like  dv ~ iv1 + iv2 + iv1 * NA
paste_na <- function(...,sep="; ", collapse=NULL) {
  L <- list(...)  # make a list of things to paste
  L <- L[!is.na(L)] # remove the NA values
  ret <- do.call(paste,c(L,list(sep=sep, collapse=collapse))) # paste them all together
  ret <- gsub(paste0('^(',sep,'){1,}|(', sep, '){1,}$'), '', ret, fixed=TRUE) # remove leading or trailing separators
  ret <- gsub(paste0('(',sep,'){2,}'), sep, ret, fixed=TRUE) # remove duplicated separators
  is.na(ret) <- ret=="" 
  ret
}
...
# if is.pois returns TRUE, give a warning to the user that their DV is poisson distributed and they should be using glm with a poisson distribution. 
